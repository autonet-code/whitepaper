...
**Substrate validation.** The post-and-coparent architecture has been validated in a four-tier experimental arc: synthetic three-root composition (6/6 architectural predictions), LLM-as-embedder for code-domain verdicts (4/5 with both small-local and frontier-tier models converging), N-agent consensus at scale (load-bearing predictions hold for N up to 1000, with sub-linear settle-time scaling and 3.4-5.0× decisiveness gain on tilted inputs), and autonet pipeline integration (4/5 with the LLM-binary-flag adapter slotting cleanly into the existing turn-to-observation seam). The combined story: the substrate produces deterministic verdicts on single-claim review, N-agent consensus, and real autonet code paths, with embedder choice (keyword heuristic vs small LLM vs frontier LLM) trading off cost against decisiveness without changing the architecture beneath it.
